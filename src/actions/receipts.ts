...
export const getReceiptById = async (id: number) => {
  ...
  if (!getReceipt) throw new Error("Receipt is not found");
...
export const createReceipt = async (
  ...
  const isItem = await Promise.all(
    items.map(async (fo) => {
      ...
      if (orderMedicine.received_total + fo.received > orderMedicine.quantity)
        throw new Error(
          `Quantity item ${orderMedicine.medicine?.name} is exceeds the limit`
        );
  ...
  const request_status =
    total_received + orderExist.total_item_received == orderExist.total_item
      ? "full"
      : "partial";
  ...
  await db.transaction(async (tx) => {
    ...
    await Promise.all(isItem.map(async (fo) => {
      ...
      if ((fo.medicine?.stock! + fo.payload.received) >= fo.medicine?.medicine_reminder.max_stock!) {
        await createNotif("stock", fo.id, {
          title: "Stok obat melebihi batas",
          description: `Stok obat ${fo.medicine?.name} mulai mebihi ${fo.medicine?.stock! + fo.payload.received} dengan batas ${fo.medicine?.medicine_reminder.max_stock}`
        })
      }
  ...
  await createActivityLog((user) => ({
    ...
    action_type: "create",
    description: `${user.name} menerima pesanan `,
...
export const removeReceipt = async (id: number) => {
  if (!id) throw new Error("Id value required");
...
export const updateReceipt = async (
  ...
  const isItem = await Promise.all(
    items.map(async (fo) => {
      ...
      const current_order_total_item =
        orderMedicine.received_total - isExist.received;
  ...
  const current_order_total_item =
    orderExist.total_item_received - isExisting.total_received_item;

  const request_status =
    current_order_total_item + total_received == 0
      ? "not_yet"
      : current_order_total_item + total_received == orderExist.total_item
        ? "full"
        : "partial";
    ...
    await Promise.all(
      isItem.map(async (fo) => {
        await tx
          ...
          .set({
            received_total:
              fo.payload.current_order_total_item + fo.payload.received,
          })
  ...
  await createActivityLog((user) => ({
    action_name: "Menerima pesanan",
    action_type: "update",
    description: `${user.name} mengupdate informasi pesanan`,
