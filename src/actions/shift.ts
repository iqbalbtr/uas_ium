...
export const createShift = async (balance_holder: string, balance: number) => {
  ...
  await createActivityLog((user) => ({
    action_name: "Membuat Shift",
    action_type: "create",
    description: `${user.name} membuat informasi shift`,
    title: "Membuat shift",
  }));
...
export const updateBalanceShift = async (balance: number) => {
  ...
  await createActivityLog((user) => ({
    action_name: "Mengupdate Shift",
    action_type: "update",
    description: `${user.name} mengupdate informasi shift`,
    title: "Mengupdate shift",
  }));
...
export const updateEndShift = async () => {
  ...
  const retur_transactions = await db.query.retur_transactions.findMany({
    where: (tr, { and, gte, lte, eq }) =>
      and(gte(tr.retur_date, startOfDay), lte(tr.retur_date, endOfDay)),
  });

  const income_total = transactions.reduce((acc, pv) => (acc += pv.total), 0);
  const retur_total = retur_transactions.reduce(
    (acc, pv) => (acc += pv.total_retur!),
    0
  );
  const retur_item_total = retur_transactions.reduce(
    (acc, pv) => (acc += pv.total_item!),
    0
  );
  ...
  await db
    ...
    .set({
      ...
      retur_item_total,
      balance_different:
        income_total - retur_total - latestShift?.cashier_balance!,
