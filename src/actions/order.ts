...
export const createOrder = async (
  ...
  if (!medicine.length) throw new Error("Medicine minimum is one");
  ...
  const totalOrder = medicine.reduce(
    (acc, prev) => (acc += prev.price * prev.qty),
    0
  );
...
export const getOrder = async (
  ...
  const result = await db.query.orders.findMany({
    ...
    where(
      { order_code, request_status, payment_status },
      { like, eq, and, ne }
    ) {
      return and(
        query
