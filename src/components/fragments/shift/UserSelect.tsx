...
export function UserSelect({
    ...
    const getData = async (name?: string) => {
        setLoading("loading");
        try {
            const get = await getUser(1, 5, name || "");
            const data = get?.data || [];
            setRoles(data.map(role => ({ value: role.username, label: role.username })));
            setLoading("success");
        } catch (error: any) {
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent className="w-[200px] p-0">
                <Command>
                    <CommandInput onInput={(e: any) => HandleSearch(e.target.value)} placeholder="Cari user..." className="h-9" />
                    <CommandList>
                        {
                            isLoading == "loading" ?
