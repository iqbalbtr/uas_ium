...
export const payment_method_enum = pgEnum("payment_method", ["cash", "installment"]);
export const order_status_enum = pgEnum("order_status", ["pending", "completed", "cancelled"]);
export const transaction_status_enum = pgEnum("transaction_status", ["pending", "completed", "cancelled"]);
export const receipt_status_enum = pgEnum("receipt_status", ["accepted", "rejected", "pending"]);
export const request_status_enum = pgEnum("request_status", ["full", "partial"]);
...
export const medicine_relations = relations(medicines, ({ one }) => ({
    medicine_reminder: one(medicine_reminder, { fields: [medicines.id], references: [medicine_reminder.medicine_id] }),
}))
...
export const order_medicine_relations = relations(order_medicine, ({ one }) => ({
    medicine: one(medicines, { fields: [order_medicine.medicine_id], references: [medicines.id] }),
    order: one(orders, { fields: [order_medicine.order_id], references: [orders.id] }),
}));
...
export const prescriptions = pgTable("prescriptions", {
    id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
    code_prescription: varchar("code_prescription", { length: 100 }).unique().notNull(),
...
export const prescription_medicine = pgTable("prescription_medicine", {
    id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
    prescription_id: integer("prescription_id").references(() => prescriptions.id, { onDelete: "cascade" }).notNull(),
    medicine_id: integer("medicine_id").references(() => medicines.id, { onDelete: "set null" }),
    quantity: integer("quantity").notNull(),
    notes: text("notes"),
});

export const prescription_medicine_relations = relations(prescription_medicine, ({ one }) => ({
    medicine: one(medicines, { fields: [prescription_medicine.medicine_id], references: [medicines.id] }),
    prescription: one(prescriptions, { fields: [prescription_medicine.prescription_id], references: [prescriptions.id] }),
}));
...
export const transactions = pgTable("transactions", {
    ...
    code_transaction: varchar("code_transaction", { length: 50 }).unique().notNull(),
...
export const transaction_item = pgTable("transaction_item", {
    ...
    sub_total: integer("sub_total").notNull(),
    medicine_id: integer("medicine_id").references(() => medicines.id, { onDelete: "set null" }),
    transaction_id: integer("transaction_id").references(() => transactions.id, { onDelete: "cascade" }).notNull(),
});

export const transaction_relations = relations(transactions, ({ many, one }) => ({
    items: many(transaction_item),
    user: one(users, { fields: [transactions.user_id], references: [users.id] }),
}));

export const transaction_item_relations = relations(transaction_item, ({ one }) => ({
    medicine: one(medicines, { fields: [transaction_item.medicine_id], references: [medicines.id] }),
    transaction: one(transactions, { fields: [transaction_item.transaction_id], references: [transactions.id] }),
}));
